 ...
 * Suitable for running a seeder from a local file in the CLI.
 */
public final class SeederFileStorage implements StorageProvider, Closeable {

  private final FileChannel channel;
  private final Sha256 verifier = new Sha256();

  public SeederFileStorage(Path existingFile) throws IOException {
    channel = FileChannel.open(existingFile, StandardOpenOption.READ);
  }

  @Override
  public void preallocateSpace(long totalSize) {
    // no-op for a read-only source file
  }
